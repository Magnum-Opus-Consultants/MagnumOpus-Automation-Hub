...
def rebuild_summary_view():
    ...
    sub = f"""(
        SELECT controlling_agent_code,
               ...
               shipment_direction AS ship_direction,
    ...
    for y in years:
        for m in range(1, 13):
            ...
            select_cols.append(
                f"ROUND(COALESCE(SUM(CASE WHEN rev_month = '{ms}' AND ship_mode IN ('LCL', 'LTL') AND COALESCE(ship_direction, '') <> 'Imports' THEN chg_m3_val END), 0))::INT AS \"{y} {mn} Chg M3\""
            )
            select_cols.append(
                f"ROUND(COALESCE(SUM(CASE WHEN rev_month = '{ms}' AND ship_mode IN ('FCL', 'FTL', 'BBK') THEN teu_val END), 0))::INT AS \"{y} {mn} TEU\""
            )
            select_cols.append(
                f"COUNT(DISTINCT CASE WHEN rev_month = '{ms}' AND ship_direction = 'Imports' THEN job_ref END) AS \"{y} {mn} IMP File count\""
            )
...
ORDER BY SUM(CASE WHEN rev_year = {current_year} THEN recognized_income END) DESC NULLS LAST;"""
    ...
    SELECT
        COALESCE(NULLIF(asr.sales_rep, ''), 'Unassigned') AS "SALES REP",
    ...
    FROM customer_spend_operational cso
    LEFT JOIN agent_sales_rep asr ON asr.org_code = cso.controlling_agent_code
    GROUP BY COALESCE(NULLIF(asr.sales_rep, ''), 'Unassigned'),
             EXTRACT(YEAR FROM CAST(NULLIF(
                 CASE WHEN job_revenue_recognition_date = 'IMM' OR job_revenue_recognition_date IS NULL
                      THEN job_opened
                      ELSE job_revenue_recognition_date END
    ...
    FROM customer_spend_operational cso
    LEFT JOIN agent_sales_rep asr ON asr.org_code = cso.controlling_agent_code
    GROUP BY controlling_customer_code, controlling_customer_name, COALESCE(NULLIF(asr.sales_rep, ''), 'Unassigned'),
    ...
    SELECT
        ...
        ROUND(COALESCE(SUM(CASE WHEN ship_mode IN ('LCL', 'LTL') AND COALESCE(ship_direction, '') <> 'Imports' THEN chg_m3_val END), 0))::INT AS "CHG M3",
